...
class FileUtils {
  ...
  /// Get all files
  static Future<List<FileSystemEntity>> getAllFilesInPath(String path, {bool showHidden = false}) async {
    List<FileSystemEntity> files = <FileSystemEntity>[];
...
//    print(files);
  ...
  static String getMime(String path) {
    File file = File(path);
    String mimeType = file.mimeType.toLowerCase();
    return mimeType;
  }

  static Future<List<FileSystemEntity>> getRecentFiles({bool showHidden = false}) async {
  ...
  static Future<List<FileSystemEntity>> searchFiles(String query, {bool showHidden = false}) async {
